...
This includes a set of Python scripts to aid in the management of GitHub App authentication workflow for either **Private Key** or User based **OAuth** workflows.

Note that the 2 authentication methods supported by this module are exclusive. **Private Key** authentication is chosen as the default.

## Private Key Authentication

To use **Private Key** based authentication, ensure that the module parameter `authentication_method` is set to `private_key`. Python scripts generated by this module will then support this workflow only.
Parameters required by this method, with their default values are as follows:
...
To use **OAuth** based authentication, ensure that the module parameter `authenticaiton_method` is set to `oauth`. The python scripts generated by this module will support this workflow only. 
Parameters required by this method, showing their defaults are as follows:

```
  Stdlib::Absolutepath $location                     = '/opt/puppetlabs/github_app_pe_oauth_management',
  Stdlib::Absolutepath $config                       = '/etc/puppetlabs/github_app_pe_oauth_management_config.json',
  Enum['private_key','oauth'] $authentication_method = 'oauth',
  Sensitive[String[1]] $github_client_id             = Sensitive('<GITHUB_CLIENT_ID>'),
...
NOTE: The Homepage and Callback URLs are not required for Private Key based authentication. When using OAuth workflow, the below URLs should NOT be accessible from GitHub or from the internet in general. They only need to be accessible from your browser within your own internal network for the OAuth workflow to succeed.
...
3\. Install the GitHub App into your organization in GitHub. Ensure both a `Client Secret` and a `Private Key` are generated. 

**For Private Key authentication:**

Make note of the `Client ID` and save the `Private Key` file as they will be needed for the next step. (Note: The `Private Key` file may be downloaded automatically when generated).
...
4\. Create encrypted hiera configuration and assign your GitHub App `Client ID` and `Client Secret` OR `Private Key` from the previous step to `github_app_pe_oauth_management::github_client_id` and `github_app_pe_oauth_management::github_client_secret` or `github_app_pe_oauth_management::github_app_private_key`.

This hiera data should be applied at the `node level` to your Puppet Enterprise **Primary server only**.

**Example hiera config for Private Key based authentication**
```
lookup_options:
 "^github_app_pe_oauth_management::github_client_id":
   convert_to: "Sensitive"
 "^github_app_pe_oauth_management::ggithub_app_private_key":
   convert_to: "Sensitive"
...
github_app_pe_oauth_management::github_app_private_key: >
...
**Example hiera config for OAuth based authentication**
```
lookup_options:
...
# Usage - Private Key Authentication Method

When the module is applied in Private Key mode with `authentication_method => 'private_key'`, a set of Python scripts are generated from EPP templates and configured for use on your Primary Puppet server.

These scripts will use the provided Private Key and interface with the GitHub App to create a token for machine use.

### 1\. Obtaining a token

The `retrieve_github_app_token.py` script will use the PyGithub library and use the provided `github_app_private_key` to generate a new token. This token will be placed in the `token_file` on disk for Puppet Enterprise Code Manger to use.
This is a long lived token, prefixed with `ghs_`.

### 2\. Checking the current token

The `check_current_token.py` script is provided for information purposes only. It will output json formatted information about the current GitHub Access Token. If this script fails, the token is likely invalid, expired or has been revoked and the workflow should be restarted at Step 1.


# Usage - OAuth Authentication Method

When the module is applied in OAuth mode with `authentication_method => 'oauth'`, a set of Python scripts are generated from EPP templates and configured for use on your Primary Puppet server.
...
This is a set of short lived tokens, prefixed with `ghu_` and `ghr_` respectively.


### 3\. Check for token freshness or expiry

The `3_check_refresh_needed.py` script is used to determine the "freshness" of the Access Token or if we are within the specified `refresh_threshold` and should refresh the token.
...
The `4_refresh_token.py` script will perform a refresh using the current Refresh Token.
...
### 5\. Checking the current token

The `check_current_token.py` script is provided for information purposes only. It will output json formatted information about the current GitHub Access Token. If this script fails, the token is likely invalid, expired or has been revoked and the workflow should be restarted at Step 1.


## OAuth Workflow Notes
